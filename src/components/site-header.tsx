      ...
      <div className="mx-auto flex min-h-20 max-w-7xl items-center justify-between gap-4 px-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label={siteContent.ui.menu[locale]}>
          {navigationKeys.map((key) => (
            <Link key={key} href={key === "home" ? `/${locale}/home` : `/${locale}/${key}`} className="rounded-full px-3 py-2 text-sm text-[var(--muted)] transition-colors hover:bg-[var(--surface)] hover:text-[var(--foreground)]">
              {siteContent.navigation[key][locale]}
            </Link>
          ))}
        </nav>
        <div className="flex items-center gap-2">
          <Link href={localizedPath} className="rounded-full px-3 py-2 text-sm font-semibold text-[var(--muted)] transition-colors hover:bg-[var(--surface)] hover:text-[var(--foreground)]" lang={otherLocale}>
            {locale.toUpperCase()}
          </Link>
      ...
      {isOpen ? (
        <nav className="border-t border-[var(--border)] px-5 py-3 lg:hidden" aria-label={siteContent.ui.menu[locale]}>
          <div className="mx-auto grid max-w-7xl gap-1">
            {navigationKeys.map((key) => (
              <Link key={key} href={key === "home" ? `/${locale}/home` : `/${locale}/${key}`} onClick={() => setIsOpen(false)} className="rounded-lg px-3 py-3 text-sm font-medium text-[var(--muted)] hover:bg-[var(--surface)] hover:text-[var(--foreground)]">
                {siteContent.navigation[key][locale]}
              </Link>
            ))}
          </div>
